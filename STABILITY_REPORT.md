...
> 작성일: 2024-12-16
> 수정일: 2024-12-16 (VPN 로그 DEBUG 모드 전환)
...
#### 수정 전 (2024-12-16)
```
logs/vpn/      5.2 GB (99%) ← VPN 디버그 로그 (불필요)
logs/기타       40 MB (1%)
browser-data/  3.4 GB
```
...
#### 수정 후 예상
```
logs/          ~50 MB/월 (akamai CSV + 에러 로그만)
browser-data/  3.4 GB (프로필 누적)
```

**예상 연간 디스크 사용량:**
- 로그: 약 **600 MB/년** (VPN 로그 비활성화 후)
...
|------|----------|--------|----------------|
| 디스크 관리 (로그) | ✅ VPN 로그 비활성화됨 | 🟢 양호 | 연 600MB 수준 |
| 디스크 관리 (프로필) | 자동 정리 없음 | 🟡 중간 | 6개월 내 누적 문제 |
| 프로세스 복구 | 자동 복구 없음 | 🟡 중간 | PM2 도입 권장 |
| 에러 핸들링 | Global 핸들러 없음 | 🟡 중간 | 권장 사항 |
...
```
수정 전: ❌ 365일 무중단 운영 부적합 (VPN 로그로 인한 디스크 풀)
수정 후: ✅ 365일 무중단 운영 가능 (VPN 로그 비활성화)
권장 사항: PM2 도입으로 안정성 강화
```
