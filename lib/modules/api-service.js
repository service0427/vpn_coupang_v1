...
class DongleAllocator {
  ...
  async toggle(serverIp, dongleNumber) {
    console.log(`🔄 동글 IP 토글 요청: http://${serverIp}/toggle/${dongleNumber}`);

    try {
      const axios = require('axios');
      // 토글은 20~60초 소요 - 완료될 때까지 대기해야 함
      // 토글 완료 후 같은 동글을 재사용할 수 있으므로 동기 처리 필요
      const response = await axios.get(`http://${serverIp}/toggle/${dongleNumber}`, {
        timeout: 90000  // 90초 타임아웃
      });
      const newIp = response.data?.ip || 'unknown';
      console.log(`✅ 동글 ${dongleNumber} 토글 완료 → 새 IP: ${newIp}`);
      return true;
    } catch (error) {
      console.log(`⚠️ 동글 ${dongleNumber} 토글 실패: ${error.message}`);
      return false;
    }
  }
