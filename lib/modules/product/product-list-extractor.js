 ...
 * Updated: 2025-12-13 - Access Denied 전용 로깅 통합
 */
 ...
 * @param {string} options.location - 감지 위치 (selector_wait, timeout_check)
 * @param {number|string} options.dongle - VPN 동글 번호
 */
async function retryOnAccessDenied(page, keywordId, threadPrefix, pageNum, maxRetries = 3, options = {}) {
  const idPrefix = createIdPrefix(keywordId);
  const { location = 'unknown', dongle = 'standard' } = options;
  const startTime = Date.now();

  // 쓰레드 번호 추출 (threadPrefix에서)
  ...
  while (retryCount < maxRetries) {
    ...
    if (!retryTitle.toLowerCase().includes('access denied')) {
      ...
      try {
        ...
        return { success: true, recoveredAtAttempt: retryCount };
      } catch (retryError) {
  ...
  if (retryCount >= maxRetries && lastError) {
    ...
    accessDeniedLogger.logFailed({
      ...
      dongle,
      keywordId,
      finalError: lastError?.message || 'Access Denied 지속',
    });
 ...
 * 페이지에서 상품 목록 추출
 * @param {Object} options - 추가 옵션
 * @param {number|string} options.dongle - VPN 동글 번호 (로깅용)
 */
async function extractProductList(page, productId, keywordId = null, threadPrefix = '', pageNum = 1, options = {}) {
  const idPrefix = createIdPrefix(keywordId);
  let { dongle = 'standard' } = options;

  // 쓰레드 번호 추출
  const threadMatch = threadPrefix.match(/\[(\d+)\]/);
  const threadNum = threadMatch ? parseInt(threadMatch[1]) : 0;

  // VPN 동글 번호 추출 시도 (threadPrefix 또는 환경변수에서)
  // 예: "[01:23:45][vpn-19][01]" 형식에서 19 추출
  if (dongle === 'standard') {
    const vpnMatch = threadPrefix.match(/\[vpn-(\d+)\]/i);
    if (vpnMatch) {
      dongle = vpnMatch[1];
    } else if (process.env.VPN_DONGLE) {
      dongle = process.env.VPN_DONGLE;
    }
  }
  ...
  try {
    ...
    if (error.errorType === 'access_denied') {
      ...
      await retryOnAccessDenied(page, keywordId, threadPrefix, pageNum, 3, { location: 'selector_wait', dongle });
      ...
      if (pageContent.hasProductList && pageContent.productCount > 0) {
        ...
        if (pageContent.title.toLowerCase().includes('access denied')) {
          ...
          await retryOnAccessDenied(page, keywordId, threadPrefix, pageNum, 3, { location: 'timeout_check', dongle });
        }
        // 점검 페이지 감지
