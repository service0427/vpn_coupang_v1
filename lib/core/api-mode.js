  ...
  constructor(config = {}) {
    ...
    for (let i = 0; i < this.threadCount; i++) {
      ...
      this.hubApiClients.set(i, new HubApiClient({
        ...
        realServerIp: process.env.REAL_IP || null  // 실제 서버 IP (환경변수로 전달받음)
      }));
    }
    ...
    // 실제 서버 IP (VPN 모드에서 환경변수로 전달받음)
    this.realServerIp = process.env.REAL_IP || null;

    console.log(`🤖 ApiModeRunner 초기화 (쓰레드: ${this.threadCount}개)`);
    if (this.vpnMode) {
      console.log(`   VPN 모드: 동글 ${this.vpnDongle}, 실제 IP: ${this.realServerIp || '미설정'}`);
    }
  }
