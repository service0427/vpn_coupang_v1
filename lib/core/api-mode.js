...
class ApiModeRunner {
  constructor(config = {}) {
    ...
    // VPN 동글 번호 추출 (vpn-16 -> 16, vpn-dyn-16 -> 16, vpn-multi-16 -> 16)
    // 또는 환경변수 VPN_DONGLE 직접 사용
    this.vpnDongle = process.env.VPN_DONGLE
      ? parseInt(process.env.VPN_DONGLE)
      : (this.vpnNamespace ? parseInt(this.vpnNamespace.replace(/^vpn-(dyn-|multi-)?/, '')) : null);
    ...
    // SharedCache 초기화 (모든 VPN이 하나의 캐시 공유)
    const sharedCacheBasePath = config.basePath || './browser-data';
    ...
    if (this.vpnMode) {
      console.log(`   VPN 모드: 동글 ${this.vpnDongle}`);
    }
  }
  ...
  async startBatchWorkLoop() {
    ...
    while (this.isRunning) {
      ...
      // 모든 쓰레드에서 시차를 두고 작업 실행
      const workPromises = [];
      // 총 시작 시간을 15초로 고정하고, 쓰레드 수로 나눔 (최소 500ms, 최대 2초)
      const totalSpreadTime = 15000; // 15초 안에 모든 쓰레드 시작
      const threadDelay = Math.max(500, Math.min(2000, Math.floor(totalSpreadTime / this.threadCount)));
      console.log(`📊 쓰레드 시차: ${threadDelay}ms × ${this.threadCount}개 = ${(threadDelay * this.threadCount / 1000).toFixed(1)}초`);
      for (let i = 0; i < this.threadCount; i++) {
        // 쓰레드별 시차 시작 (브라우저 동시 시작 방지)
        const delay = i * threadDelay;
  ...
  async startThreadWorkLoop(threadIndex) {
    ...
    tsLog(`🔧 쓰레드 ${threadNumber} 워크루프 시작`);
    ...
    // 쓰레드별 시차 시작 (15초 안에 모든 쓰레드 분산, 최소 500ms ~ 최대 2초)
    const totalSpreadTime = 15000;
    const threadDelay = Math.max(500, Math.min(2000, Math.floor(totalSpreadTime / this.threadCount)));
    setTimeout(() => {
      if (this.isRunning) {
        threadWorkLoop();
      }
    }, threadIndex * threadDelay);
  }
  ...
  async processNextWork(threadIndex) {
    ...
    try {
      ...
      tsLog(`   📡 [쓰레드 ${threadNumber}] 작업 할당 요청 중...`);
      const allocateStart = Date.now();
      const workAllocation = await hubApiClient.allocateWork();
      tsLog(`   📡 [쓰레드 ${threadNumber}] 작업 할당 응답 (${Date.now() - allocateStart}ms)`);
      ...
      if (!workAllocation) {
        ...
        if (this.options.once) {
          ...
          if (this.completedThreads >= this.threadCount) {
            ...
            console.log(`🛑 --once 모드: 프로그램 종료 (차단: ${this.stats.blocked}개)`);
            this.stop();
            // 종료 코드로 차단 횟수 전달 (VPN 모드에서 IP 토글 판단용)
            process.exit(Math.min(this.stats.blocked, 255));
          }
          return;
        }
  ...
  async executeWork(workAllocation, threadIndex) {
    ...
    tsLog(`▶️ [${modeLabel}] 작업 실행 시작: ${workAllocation.work.keyword} (workType: ${workAllocation.work.workType || 'click'})`);
    ...
    try {
      ...
      // VPN 모드: threadIndex가 각 쓰레드 번호 (0부터 시작)
      const { calculateViewportSize } = require('../utils/browser-helpers');
      const layoutIndex = threadIndex;  // 쓰레드 인덱스 그대로 사용 (VPN 동글은 환경변수로 전달)
      ...
      // (VPN별 + 쓰레드별로 프로필 폴더가 분리되므로 Chrome 버전 겹쳐도 OK)
      if (!this.options.chromeVersion) {
        const excludedBuilds = workAllocation.work.excludedChromeBuilds || [];
        const selectedChrome = getRandomChromeVersion(excludedBuilds);

        if (selectedChrome) {
          executablePath = selectedChrome.path;
          actualChromeVersion = selectedChrome.version;
          console.log(`   🎲 [${modeLabel}] 랜덤 Chrome 선택: ${actualChromeVersion} (${selectedChrome.dir})`);
        } else {
          console.log(`   ⚠️ [${modeLabel}] 설치된 Chrome 버전이 없습니다. 기본 Chrome 사용`);
        }
      } else {
      ...
      const basePath = process.cwd();  // 현재 작업 디렉토리 사용
      if (this.vpnMode && this.vpnDongle) {
        userFolderPath = `${basePath}/browser-data/vpn_${this.vpnDongle}/${folderNumber}/${chromeMajorVersion}`;
        console.log(`   📁 [${modeLabel}] 프로필 폴더: vpn_${this.vpnDongle}/${folderNumber}/${chromeMajorVersion}`);
      } else {
        userFolderPath = `${basePath}/browser-data/${folderNumber}/${chromeMajorVersion}`;
      ...
      // 오래된 파일 정리 (3일 초과, 폴더 구조 유지)
      try {
        const { cleanOldFiles } = require('../../utils/browser-data-cleaner');
        cleanOldFiles(userFolderPath, { maxAgeDays: 3 });
      } catch (cleanError) {
        // 정리 실패는 무시
      }
      ...
      // 세션 관리용 폴더 키 생성
      const folderKey = this.vpnMode && this.vpnDongle
        ? `vpn_${this.vpnDongle}/${folderNumber}/${chromeMajorVersion}`
        : `${folderNumber}/${chromeMajorVersion}`;

      // 브라우저 실행 (항상 GUI 모드)
      // 첫 실행 시에만 동시 브라우저 시작으로 인한 크래시 방지 - 쓰레드별 2초 지연
      if (!this.threadFirstRun.has(threadNumber)) {
        const browserStartDelay = (threadNumber - 1) * 2000;  // 2초 간격으로 시작
        if (browserStartDelay > 0) {
          tsLog(`   ⏳ [쓰레드 ${threadNumber}] 브라우저 시작 대기 ${browserStartDelay / 1000}초...`);
          await new Promise(resolve => setTimeout(resolve, browserStartDelay));
        }
        this.threadFirstRun.add(threadNumber);
      }

      tsLog(`   🚀 [쓰레드 ${threadNumber}] 브라우저 실행 시작... (프로필: ${folderNumber}/${chromeMajorVersion})`);
      ...
      for (let attempt = 1; attempt <= 2; attempt++) {
        try {
          tsLog(`   🔧 [쓰레드 ${threadNumber}] getBrowser 호출 시작 (시도 ${attempt}/2)`);
          ...
          tsLog(`   ✅ [쓰레드 ${threadNumber}] getBrowser 완료 (${Date.now() - getBrowserStart}ms)`);
          break; // 성공하면 루프 종료
        } catch (err) {
          lastError = err;
          // 타임아웃 또는 에러 발생 시 해당 프로필의 Chrome 프로세스 강제 종료
          tsLog(`   🔪 [쓰레드 ${threadNumber}] 브라우저 시작 실패 - 좀비 프로세스 정리 중...`);
          try {
            const { execSync } = require('child_process');
            // 1. 해당 프로필 폴더를 사용하는 Chrome 프로세스 찾아서 종료
            execSync(`pkill -9 -f "${userFolderPath}" 2>/dev/null || true`, { stdio: 'pipe' });
            // 2. SingletonLock 파일 삭제 (다음 시작을 위해)
            execSync(`rm -f "${userFolderPath}/SingletonLock" 2>/dev/null || true`, { stdio: 'pipe' });
            execSync(`rm -f "${userFolderPath}/SingletonCookie" 2>/dev/null || true`, { stdio: 'pipe' });
            execSync(`rm -f "${userFolderPath}/SingletonSocket" 2>/dev/null || true`, { stdio: 'pipe' });
          } catch (killErr) {
            // 무시
          }
          if (attempt < 2) {
            tsLog(`   ⚠️ [쓰레드 ${threadNumber}] 브라우저 시작 실패: ${err.message}, 3초 후 재시도...`);
      ...
      tsLog(`   📄 [쓰레드 ${threadNumber}] 페이지 객체 획득 완료`);

      // about:blank 상태면 새로고침으로 깨우기 (최대 3회, 3초 간격)
      let currentUrl = page.url();
      if (currentUrl === 'about:blank' || !currentUrl) {
        let reloadSuccess = false;
        for (let reloadAttempt = 1; reloadAttempt <= 3; reloadAttempt++) {
          tsLog(`   🔄 [쓰레드 ${threadNumber}] about:blank 감지 - 새로고침 시도 ${reloadAttempt}/3...`);
          try {
            await page.reload({ timeout: 5000 }).catch(() => { });
            await new Promise(r => setTimeout(r, 3000)); // 3초 대기
            currentUrl = page.url();
            if (currentUrl !== 'about:blank' && currentUrl) {
              tsLog(`   ✅ [쓰레드 ${threadNumber}] 새로고침 성공 (${currentUrl})`);
              reloadSuccess = true;
              break;
            }
          } catch (e) {
        ...
        if (!reloadSuccess) {
          tsLog(`   ❌ [쓰레드 ${threadNumber}] 새로고침 3회 실패 - 브라우저 종료`);
          try {
            if (browser) await browser.close().catch(() => { });
            const { execSync } = require('child_process');
            execSync(`pkill -9 -f "${userFolderPath}" 2>/dev/null || true`, { stdio: 'pipe' });
          } catch (e) { }
          throw new Error('브라우저 무응답 - about:blank 상태 지속');
        }
      }
      tsLog(`   ✅ [쓰레드 ${threadNumber}] 브라우저 준비 완료`);
      ...
      if (!actualChromeVersion || actualChromeVersion === null) {
        try {
          ...
          if (browserVersion && !browserVersion.includes('/')) {
            ...
            tsLog(`   📱 [쓰레드 ${threadNumber}] 감지된 Chrome 버전: ${actualChromeVersion}`);
          } else {
            // 다른 형식일 경우 파싱
