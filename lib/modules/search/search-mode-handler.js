...
async function executeDirectMode(page, searchQuery, options = {}) {
  ...
  if (options.directUrl) {
    ...
    try {
      ...
      // 메인 페이지 Access Denied 감지 + 새로고침 재시도
      {
        const mainTitle = await page.title().catch(() => '');
        if (mainTitle.toLowerCase().includes('access denied')) {
          console.log(`${threadPrefix} ${idPrefix}🚫 메인 페이지 Access Denied 감지 - 새로고침 시도`);

          const mainMaxRetries = 3;
          const mainRetryDelay = 5000;
          let mainRecovered = false;
          ...
          for (let mRetry = 1; mRetry <= mainMaxRetries; mRetry++) {
            console.log(`${threadPrefix} ${idPrefix}🔄 메인 페이지 새로고침 (${mRetry}/${mainMaxRetries})...`);
            ...
            await page.waitForTimeout(mainRetryDelay);
            await page.reload({ waitUntil: 'domcontentloaded', timeout: 20000 });

            const retryTitle = await page.title().catch(() => '');
            if (!retryTitle.toLowerCase().includes('access denied')) {
              console.log(`${threadPrefix} ${idPrefix}✅ ${mRetry}번째 새로고침으로 메인 페이지 복구`);
              accessDeniedLogger.logRecovered({
                attemptNum: mRetry,
                threadNum,
                dongle,
                keywordId: options.keywordId,
                recoveryTimeMs: mRetry * mainRetryDelay,
              });
              mainRecovered = true;
              break;
            }
          ...
          if (!mainRecovered) {
            accessDeniedLogger.logFailed({
              ...
              dongle,
              keywordId: options.keywordId,
              finalError: 'Access Denied - 메인 페이지 새로고침 재시도 초과',
            });
            const blockError = new Error('쿠팡 메인 페이지 Access Denied - 재시도 초과');
    ...
    try {
      ...
      // 메인 페이지 Access Denied 감지 + 새로고침 재시도
      {
        const mainSearchTitle = await page.title().catch(() => '');
        if (mainSearchTitle.toLowerCase().includes('access denied')) {
          console.log(`${threadPrefix} ${idPrefix}🚫 메인 페이지 Access Denied 감지 - 새로고침 시도`);
          ...
          accessDeniedLogger.logDetected({
            location: 'main_page_search',
            threadNum,
            dongle,
            keywordId: options.keywordId,
            url: mainUrl,
            pageTitle: mainSearchTitle,
          });
