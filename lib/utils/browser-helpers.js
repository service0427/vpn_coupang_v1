...
// VPN 멀티 모드 창 설정 (10개 VPN, 2행 5열 배치)
...
const VPN_MULTI_COLS = 5;  // 5열 (최대 5 VPN)
...
const VPN_MULTI_Y_SPACING = 60;  // 쓰레드별 세로 간격 (60px)
...
function calculateVpnWindowPosition(vpnDongle) {
  // VPN 인덱스 (0~4) - VPN_INDEX 환경변수 우선 사용
  let vpnIndex;
  if (process.env.VPN_INDEX) {
    vpnIndex = parseInt(process.env.VPN_INDEX) - 1;  // 1-based → 0-based (0~4)
  } else {
  ...
  // X 위치: VPN별 열 (60, 580, 1100, 1620, 2140)
  const col = vpnIndex % VPN_MULTI_COLS;
  const x = VPN_MULTI_X_START + (col * VPN_MULTI_X_SPACING);

  // Y 위치: 쓰레드별 60px 간격 (20, 80, 140, 200, 260, 320, 380, 440)
  const y = VPN_MULTI_Y_START + (threadIndex * VPN_MULTI_Y_SPACING);

  console.log(`📍 VPN${vpnIndex + 1}-T${threadIndex + 1}: 위치(${x}, ${y})`);
