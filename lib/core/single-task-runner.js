...
async function main() {
  ...
  try {
    ...
    // 5. 캐시 공유 설정 (심볼릭 링크)
    try {
      const sharedCacheManager = new SharedCacheManager({ basePath: path.join(process.cwd(), 'browser-data') });
      await sharedCacheManager.initialize();
      const isFirstRun = await sharedCacheManager.isFirstRun(userFolderPath);
      await sharedCacheManager.setupUserFolderCache(userFolderPath, isFirstRun, false);
      log(`🔗 캐시 공유 설정 완료: ${folderKey}`);
    } catch (cacheErr) {
      // 캐시 설정 실패는 무시 (작업에 영향 없음)
      log(`⚠️ 캐시 공유 설정 실패 (무시): ${cacheErr.message}`);
    }
    ...
    browserInfo = await browserCore.getBrowser({
      ...
      clearSession: true  // 매 작업마다 세션 초기화
    });

    const page = browserInfo.page;

    // 9. 검색 실행 (executeKeywordSearch 내부에서 IP 체크 후 검색 페이지로 이동)
    ...
    // 10. 결과 처리
    // searchResult: { success, productFound, errorType, errorMessage }
    // - success: true + productFound: true → 완전 성공
    // - success: true + productFound: false → 검색 성공, 상품 없음 (NOT_FOUND)
    // - success: false → 에러 발생
    ...
    } else if (searchResult.success && !searchResult.productFound) {
      // 검색 성공, 상품 없음 → NOT_FOUND (성공 처리)
      log(`📭 검색 성공, 상품 없음 → NOT_FOUND`);
      const errorMessage = searchResult.errorMessage || '상품을 찾을 수 없음';
      outputResult(createFailureResult(task.allocationKey, 'NOT_FOUND', errorMessage));
    ...
    if (errMsg.includes('http2') || errMsg.includes('net::err_http2_protocol_error')) {
      ...
      errorType = 'BLOCKED';  // SSL/TLS 에러도 BLOCKED 처리
    ...
    // --once 모드: 브라우저 종료 전 엔터 입력 대기
    if (process.env.ONCE_MODE === 'true' && browserInfo && browserInfo.browser) {
      log(`\n🔍 --once 모드: 브라우저가 열려있습니다. 엔터를 누르면 종료합니다...`);
      await waitForEnter();
    }
