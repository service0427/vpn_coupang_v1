...
class BatchAllocator {
  ...
  async allocateBatch() {
    ...
    console.log(`📋 배치 할당 요청`);
    console.log(`   → POST ${this.allocateUrl}/allocate`);
    ...
    try {
      ...
      console.log(`✅ 배치 할당 성공: ${data.count || tasks.length}개 작업`);

      // 각 작업 정보 로깅
      tasks.forEach((task, idx) => {
        const keywordShort = task.keyword.length > 30
          ? task.keyword.substring(0, 30) + '...'
          : task.keyword;
        console.log(`   [${idx + 1}] ${keywordShort} (${task.product_id})`);
      });
...
class DongleAllocator {
  ...
  static createWgConfig(dongle) {
    return {
      ...
      address: `10.8.${dongle.dongleNumber}.0/24`  // 동글 번호로 계산
    };
  }
}
