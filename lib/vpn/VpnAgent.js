...
class VpnAgent {
  ...
  async runIndependentLoop() {
    ...
    while (!this.shouldStop) {
      ...
      // 작업 없음 처리: IP_ALL_USED는 즉시 토글, 나머지는 60초 대기
      if (!hasWork && result.noWorkReason) {
        if (result.noWorkReason === 'IP_ALL_USED') {
          ...
          // 그 외 (ALL_QUOTA_REACHED, NO_ACTIVE_TASKS 등): 60초 대기
          const waitSec = result.noWorkWaitSeconds || 60;
          this.log(`⏳ 작업 없음 (${result.noWorkReason}) - ${waitSec}초 대기 후 재시도...`);
          await new Promise(r => setTimeout(r, waitSec * 1000));
          continue;  // 토글 체크 건너뛰고 바로 다음 사이클
        }
      } else if (hasWork) {
        this.noWorkCount = 0;
      }
