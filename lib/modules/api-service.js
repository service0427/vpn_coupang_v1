...
class BatchAllocator {
  ...
  async allocateBatch(count) {
    // count 파라미터: 반드시 명시적으로 전달 (--threads 값)
    const requestCount = count || this.batchSize || 5;

    const requestBody = {
      agent_ip: this.agentIp,
      vpn: this.vpnId,
      external_ip: this.externalIp,
      count: requestCount  // 항상 명시적으로 전송
    };

    console.log(`📋 배치 할당 요청: POST ${this.allocateUrl}/allocate (count=${requestCount})`);
  ...
  static createClickSuccess(allocationKey, extras = {}) {
    ...
    if (extras.duration_ms) {
      result.duration_ms = extras.duration_ms;
    }
   ...
   */
  static createClickFailure(allocationKey, errorType, errorMessage, extras = {}) {
    const result = {
      allocation_key: allocationKey,
      success: false,
...
class DongleAllocator {
   ...
   * @param {Object} stats - 세션 통계 (선택)
   *   - session_duration_ms: 세션 유지 시간 (ms)
   *   - toggle_count: IP 토글 횟수
   *   - toggle_reasons: { BLOCKED, PREVENTIVE, NO_WORK, IP_CHECK_FAILED, MANUAL }
   *   - connect_attempts: 연결 시도 횟수
   *   - connect_successes: 연결 성공 횟수
   *   - avg_connect_time_ms: 평균 연결 시간 (ms)
   *   - release_reason: 반납 사유
   * @returns {boolean} 성공 여부
   */
  async release(agentId, dongleId, stats = null) {
    ...
    // 통계 정보가 있으면 추가
    if (stats) {
      requestBody.session_duration_ms = stats.session_duration_ms || 0;
      requestBody.toggle_count = stats.toggle_count || 0;
      requestBody.toggle_reasons = stats.toggle_reasons || {};
      requestBody.connect_attempts = stats.connect_attempts || 0;
      requestBody.connect_successes = stats.connect_successes || 0;
      requestBody.avg_connect_time_ms = stats.avg_connect_time_ms || 0;
      requestBody.release_reason = stats.release_reason || '명시적 반납';
    }

    console.log(`📋 동글 반납 요청: POST ${this.baseUrl}${url}`);
    if (stats) {
      console.log(`   통계: 세션 ${Math.round((stats.session_duration_ms || 0) / 1000)}초, 토글 ${stats.toggle_count || 0}회, 사유: ${stats.release_reason || '-'}`);
    }
    ...
    try {
      ...
      // 실패해도 클라이언트는 계속 진행 (서버가 타임아웃으로 자동 정리)
      return false;
    }
  }
