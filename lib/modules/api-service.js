  ...
  async allocateBatch() {
    ...
    try {
      ...
      if (!data || !data.success) {
        console.log(`❌ 할당 실패: ${data?.message || '응답 없음'}`);
        return [];
      }
...
class DongleAllocator {
   ...
   * @param {number} dongleId - (선택) 특정 동글 ID만 반납
   * @returns {boolean} 성공 여부
   */
  async release(agentId, dongleId = null) {
    const target = dongleId ? `agent_id=${agentId}, dongle_id=${dongleId}` : `agent_id=${agentId}`;
    console.log(`📋 동글 반납 요청: ${target}`);

    try {
      const payload = { agent_id: agentId };
      if (dongleId) {
        payload.dongle_id = dongleId;
      }

      const response = await this.httpClient.post('/dongle/release', payload);
      const data = response.data;

      if (data?.success) {
        console.log(`✅ 동글 반납 성공: ${target}`);
        return true;
      } else {
        console.log(`⚠️ 동글 반납 응답: ${data?.message || 'unknown'}`);
        return false;
      }
  ...
  /**
   * 동글 사용 연장 (heartbeat)
   * 타임아웃 방지를 위해 주기적으로 호출
   *
   * @param {string} agentId - 에이전트 ID
   * @param {number} dongleId - 동글 ID
   * @returns {boolean} 성공 여부
   */
  async extend(agentId, dongleId) {
    try {
      const response = await this.httpClient.post('/dongle/extend', {
        agent_id: agentId,
        dongle_id: dongleId
      });
      ...
      // extend 실패는 치명적이지 않으므로 조용히 실패
      console.error(`⚠️ 동글 연장 실패: ${error.message}`);
      return false;
    }
  }
  ...
  /**
   * 동글 IP 토글 요청
   * 차단이 많거나 예방적 IP 변경 시 사용
   *
   * @param {string} agentId - 에이전트 ID
   * @param {number} dongleId - 동글 ID
   * @returns {boolean} 성공 여부
   */
  async toggle(agentId, dongleId) {
    console.log(`🔄 동글 IP 토글 요청: agent_id=${agentId}, dongle_id=${dongleId}`);

    try {
      const response = await this.httpClient.post('/dongle/toggle', {
        agent_id: agentId,
        dongle_id: dongleId
      });
      ...
      const data = response.data;
      if (data?.success) {
        console.log(`✅ 동글 IP 토글 성공`);
        return true;
      } else {
        console.log(`⚠️ 동글 IP 토글 응답: ${data?.message || 'unknown'}`);
        return false;
      }

    } catch (error) {
      console.error(`❌ 동글 IP 토글 실패: ${error.message}`);
      return false;
    }
  }
  ...
  static createWgConfig(dongle) {
    return {
      ...
      address: `10.8.${dongle.dongleNumber}.2/24`  // 클라이언트 IP (0=네트워크, 1=서버, 2=클라이언트)
    };
  }
}
