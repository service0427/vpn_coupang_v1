...
class BrowserCore {
  ...
  async launch(options = {}) {
    const {
      ...
      executablePath = null,
      stealth = true  // [PATCH] 기본값 true로 변경 (Akamai 우회)
    } = options;
    ...
    console.log(`   - 스텔스 모드: ${stealth ? '✅ 활성' : '❌ 비활성'}`);
    ...
    // [PATCH] 랜덤 User-Agent 생성
    const userAgent = options.userAgent || this._getRandomUserAgent();
    ...
    context = await chromium.launchPersistentContext(userDataDir, {
      ...
      proxy: proxyConfig,
      userAgent: userAgent, // [PATCH] UA 적용
    ...
    if (stealth) {
      tsLog(`🕵️ 스텔스 스크립트 주입...`);
      const scripts = this._getStealthInitScripts();
  ...
  _getRandomUserAgent() {
    ...
    return RECENT_USER_AGENTS[Math.floor(Math.random() * RECENT_USER_AGENTS.length)];
  }

  /**
   * 고급 스텔스 init 스크립트 반환
   * (Canvas/Audio/WebGL/Navigator 핑거프린트 랜덤화)
   */
  _getStealthInitScripts() {
    return [
      // 1. Webdriver 속성 은폐
      `() => {
      ...
      // 2. Hardware Concurrency 랜덤화 (4~16)
      `() => {
        const cores = [4, 6, 8, 12, 16];
        const randomCore = cores[Math.floor(Math.random() * cores.length)];
        Object.defineProperty(navigator, 'hardwareConcurrency', { get: () => randomCore });
      }`,

      // 3. Canvas Fingerprint 노이즈 추가
      `() => {
        const toBlob = HTMLCanvasElement.prototype.toBlob;
        const toDataURL = HTMLCanvasElement.prototype.toDataURL;
        const getImageData = CanvasRenderingContext2D.prototype.getImageData;
        const noise = Math.random() * 0.002 - 0.001; // -0.001 ~ 0.001

        HTMLCanvasElement.prototype.toBlob = function(callback, type, quality) {
          // 호출 스택 분석 등은 복잡하므로 단순 노이즈만
          return toBlob.apply(this, [callback, type, quality]);
        };

        HTMLCanvasElement.prototype.toDataURL = function(type, quality) {
          // 단일 픽셀 조작은 성능 영향이 있으므로 생략하거나 미세 조정 가능
        ...
        CanvasRenderingContext2D.prototype.getImageData = function(x, y, w, h) {
          ...
          // 첫 번째 픽셀의 B 채널에만 미세 노이즈 추가
          if (data.length > 2) {
             data[2] = Math.max(0, Math.min(255, data[2] + Math.floor(noise * 3)));
          }
          return imageData;
        };
      }`,

      // 4. WebGL Vendor/Renderer 랜덤화
      `() => {
        ...
        WebGLRenderingContext.prototype.getParameter = function(parameter) {
          ...
          if (parameter === 37446) { // UNMASKED_RENDERER_WEBGL
            ...
            // 세션마다 고정된 값을 쓰기 위해 localStorage 등 활용 가능하나
            // 여기서는 심플하게 랜덤 (매번 바뀌어도 됨)
