...
async function main() {
  ...
  try {
    ...
    if (searchResult.success && searchResult.productFound) {
      ...
      // errorType 매핑: searchResult.errorType → 결과 에러 타입
      const errorType = searchResult.errorType === 'blocked' || searchResult.errorType === 'BLOCKED' ? 'BLOCKED' :
                        searchResult.errorType === 'timeout' ? 'TIMEOUT' :
                        'NOT_FOUND';
    ...
    if (error.message.includes('HTTP2') || error.message.includes('net::ERR_HTTP2_PROTOCOL_ERROR')) {
      errorType = 'BLOCKED';
    } else if (error.message.includes('Timeout') || error.message.includes('timeout')) {
      errorType = 'TIMEOUT';
    } else if (error.message.includes('Akamai') || error.message.includes('403')) {
      errorType = 'AKAMAI';
    } else if (error.message.includes('Navigation')) {
      errorType = 'NAVIGATION_ERROR';
    } else if (error.message.includes('Target closed') || error.message.includes('Target crashed')) {
