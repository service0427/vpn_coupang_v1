...
// [PATCH] Akamai 우회를 위해 이미지/정적 리소스 차단 해제
...
/**
 * 세션 종료 처리 (API 전송 전 호출)
 */
function finalizeSessionEvents(allocationKey) {
  // 세션 데이터는 getSessionEventResults()로 API에 전송됨
  // CSV 파일 저장은 제거됨 - 10002 포트로 전송하여 서버에서 처리
}
    ...
    if (url.includes('ljc.coupang.com') && request.method() === 'POST') {
      try {
        ...
        if (postData) {
          ...
          // [PATCH] memberSrl 강제 주입 비활성화 (트릭 막힘)
          /*
          ...
          */
          let modified = false; // 주입 로직 비활성화로 항상 false
