...
class VpnAgent {
  ...
  constructor(vpnManager, options) {
    ...
    // JSON 상태 저장 설정
    this.statusDir = path.join(__dirname, '..', '..', 'browser-data', 'vpn-status');
    this._ensureStatusDir();
  ...
  /**
   * JSON 상태 파일 저장
   * @param {string} status - 현재 상태 (idle, working, toggling, waiting)
   * @param {Object} extra - 추가 정보
   */
  _saveStatus(status = 'idle', extra = {}) {
    try {
      const dongle = this.vpnManager.getDongleNumber();
      const statusFile = path.join(this.statusDir, `vpn_${dongle}.json`);

      const data = {
        agentId: this.agentId,
        dongle,
        vpnIndex: this.vpnIndex,
        ip: this.vpnManager.getVpnIp() || 'unknown',
        status,
        stats: { ...this.stats },
        totalStats: { ...this.totalStats },
        score: this.score,
        successSinceToggle: this.successSinceToggle,
  ...
  async executeTaskInNamespace(task, threadNum) {
    ...
    return new Promise((resolve) => {
      ...
      const taskEnv = {
        ...
        ONCE_MODE: this.onceMode ? 'true' : 'false',
        DISPLAY: ':0',
        HOME: '/home/tech',
        USER: 'tech',
      ...
      // --once 모드이고 쓰레드 1개일 때 stdin 상속 (엔터 입력 대기용)
      const useInheritStdin = this.onceMode && this.maxThreads === 1;
      ...
      // 하드 타임아웃 처리 (--once 모드이고 쓰레드 1개일 때는 비활성화)
      let timeoutId = null;
      if (!useInheritStdin) {
        timeoutId = setTimeout(() => {
          ...
          this.log(`[T${threadNum}] ⏰ 하드 타임아웃 (180초 초과) - 강제 종료`);
  ...
  async runBatchCycle() {
    ...
    // 상태 저장: 사이클 시작
    this._saveStatus('working', { cycle: runNum });
    ...
    if (!tasks || tasks.length === 0) {
      ...
      // IP_ALL_USED: 토글 필요, 그 외: 10초 대기
      const needsToggle = allocReason === 'IP_ALL_USED';
      const waitSeconds = needsToggle ? 0 : 10;
    ...
    // 상태 저장: 사이클 완료
    this._saveStatus('idle', { cycle: runNum, lastCycleStats: { ...this.stats } });
  ...
  async runIndependentLoop() {
    ...
    while (!this.shouldStop) {
      ...
      if (result.noWorkReason) {
        this.log(`[DEBUG] noWorkReason=${result.noWorkReason}, hasWork=${hasWork}`);
      }
      ...
      if (toggleCheck.toggle) {
        ...
        // 상태 저장: 토글 진행 중
        this._saveStatus('toggling', { toggleReason: toggleCheck.reason });
